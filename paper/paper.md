...
The original viralrecall tool written by F.O.A has been immensely useful to the giant virus research community amassing 59 citations. However, it's drawbacks limit it's usage with the increasingly long eukaryotic genome sequences and metagenomes becoming available. Due to the design choices mentioned in the previous section, we have massively improved on the speed of v3.0 with a 6X quicker process runtime compared to v2.0 \autoref{fig:Figure}. As modern HPC systems are extremely powerful and memory-rich, the new tool is also able to achieve 2X higher CPU utilization \autoref{fig:Figure}. We do have 10X higher memory utilization in v3.0 \autoref{fig:Figure} but this is due to loading the GVOG database in memory to achieve quicker searches. However, the memory requirement is easily available on most modern systems and thus we don't consider it as a drawback.
![Process Runtime in seconds, Memory Utilization, and CPU Utilization comparison of Viralrecall v2.0 vs v3.0 (Current version). \label{fig:Figure}](./speed_mem_cpu_table.png){ width = 20% }
