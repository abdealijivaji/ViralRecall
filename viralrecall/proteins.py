...
def predict_proteins(
    input: Fasta, contigs: list[str], outbase: Path
    ...
    header: list[tuple] = []
    Header = namedtuple(
        "Header", ["contig", "query", "pstart", "pend", "pstrand", "gen_code"]
    )

    with prot, cds, gff, gbk:
        for seqrecord in contigs:
            sequence = bytes(
                str(input[seqrecord]), "UTF-8"
            )  # loading the sequence as bytes
            ...
            for n, gene in enumerate(genes, start=1):
                ...
                if len(aa) < 100000:  # HMMER can't take proteins longer than 100k aa
                    proteins.append(
                        easel.TextSequence(name=bytes(prot_id, "UTF-8"), sequence=aa)
                    )
    return proteins, header


def subset_proteins(proteins: easel.TextSequenceBlock, prot_ids):
    """Returns a subset of proteins from the TextSequenceBlock based on a list of protein IDs"""

    for prot_id in prot_ids:
        sub_prot = easel.TextSequence(
            name=proteins.indexed[prot_id].name,
...
def search_with_pyhmmer(
    proteins: easel.TextSequenceBlock, hmm_path: Path, evalue: float
):
        ...
        for hit in hitlist:
            if hit.included:
                ...
                hitlist_name = (
                    ...
                    if hasattr(hitlist.query.name, "decode")
                    else hitlist.query.name
                )
                eval = "%.3g" % hit.evalue
                results.append(
                    Result(Contig, hmm_hit, hitlist_name, round(hit.score, 2), eval)
                )
    return results
