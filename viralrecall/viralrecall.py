...
def parse_args(argv=None):
    ...
    args_parser = argparse.ArgumentParser(
        ...
        prog="viralrecall",
        add_help=False,
        description="Viralrecall: A command-line tool for predicting NCLDV-like regions in genomic data \nAbdeali (Ali) M. Jivaji, Virginia Tech Department of Biological Sciences <abdeali@vt.edu>",
        epilog="*******************************************************************\n\n*******************************************************************",
    )
    ...
    required.add_argument("-o", "--outdir", required=True, help="Output directory name")
    required.add_argument(
        "-d",
        "--database",
        required=True,
    ...
    optional.add_argument(
        ...
        required=False,
        default=int(15),
        help="sliding window size to use for detecting viral regions (default=15 kb)",
    )
    optional.add_argument(
        "-m",
        "--minsize",
        required=False,
        default=int(10),
        help="minimum length of viral regions to report, in kilobases (default=10 kb)",
    )
    optional.add_argument(
        "-s",
        "--minscore",
        required=False,
        default=int(10),
        help="minimum score of viral regions to report, with higher values indicating higher confidence (default=10)",
    )
    optional.add_argument(
        "-e",
        "--evalue",
        required=False,
        default=str(1e-10),
        help="e-value that is passed to pyHmmer for hmmsearch (default=1e-10)",
    )
    optional.add_argument(
        "-g",
        "--minhit",
        required=False,
        default=int(4),
        help="minimum number of unique viral hits that each viral region must have to be reported (default=4)",
    )
    optional.add_argument(
        "-c",
        "--cpus",
        required=False,
        default=None,
        help="number of cores to use in batch mode (default=all available cores)",
    )
    optional.add_argument(
        "-h", "--help", action="help", help="show this help message and exit"
    )
    optional.add_argument(
        "-v", "--version", action="version", version=f"ViralRecall {__version__}"
    )
...
def run_program(
    ...
    logger.info(
        f"Running viralrecall on {input.name} and the output is being deposited in {out_base.parent}"
    )
    ...
    if not filt_contig_list:
        logger.error(
            f"No contigs longer than the minimum phage size of {phagesize} bp were found in {input.name}.\n"
    ...
    hmm_df = pd.DataFrame(
        hmm_results
    )  # namedtuples perfectly compatible with pandas dataframe

    ncldv_hits = search_with_pyhmmer(proteins, ncldv_hmm, evalue)
    ncldv_hmm_results = parse_hmmer(ncldv_hits, out_base)
    ncldv_hmm_df = pd.DataFrame(
        ncldv_hmm_results, columns=["contig", "query", "HMM_hit", "bitscore", "evalue"]
    )

    # keep only top hits for each protein
    best_hits = (
        hmm_df.sort_values(by=["query", "bitscore"], ascending=False)
    ...
    df.fillna(
        {"HMM_hit": "no_hit"}, inplace=True
    )  # add column names to dictionary here to replace NA with no_hit for colums with strings
    ...
    viral_indices = extract_reg(
        window, phagesize, minscore, filt_contig_list, df, minhit
    )
    viral_coords = {}
    ...
    for key, value in viral_indices.items():
        if len(value) >= 1:
            ...
            for idx, coords in enumerate(value, start=1):
                ...
                vregion_df.insert(
                    0, "Viral_region_number", f"vregion_{idx}", allow_duplicates=True
                )
                ...
                # Run TIGTIG on the viral region to predict taxonomic classification.
                # Provide the sequence of the viral region (as string)and the viral_df of HMM hits for that region as input to TIGTOG,
                # along with the database directory. The relative paths from the database directory are hardcoded in the tigtog.py file.

                if if_tigtog:
                    logger.info(
                        f"Running Tigtog on viral region {idx} of contig {key} to predict taxonomic classification"
                    )
                    Ord_pred, Ord_prob = run_tigtog(vreg_seq, vregion_df, database)
                else:
                    Ord_pred, Ord_prob = "NA", "NA"

                vprots = ncldv_hmm_df.loc[
                    ncldv_hmm_df["query"].isin(vregion_df["query"])
                ]["HMM_hit"]
            ...
            vir_summary.append(
                Summary(
                    ...
                    num_viral_hits="NA",
                    score=str(0),
                    NCLDV_markers=str_hits(
                        ncldv_hmm_df[ncldv_hmm_df["query"] == key]["HMM_hit"], "NCLDV"
                    ),
                    Mirusvirus_hits=str_hits(
                        df[df["query"] == key]["HMM_hit"], "Mirus"
                    ),
                    Predicted_order="NA",
                    Order_confidence="NA",
                )
    ...
    logger.info(
        f"Finished running Viralrecall on {input.name} and found {count} viral region/s"
    )
    return
...
def main(argv=None):
    ...
    if not check_version(
        database
    ):  # Old version of viralrecall database does not have TIGTOG and thus incompatible.
        print(
            f"Database version is not compatible with ViralRecall version {__version__}. TIGTOG analysis will not be performed.\nPlease update the database using viralrecall_database script to run TIGTOG analysis."
        )
        if_tigtog = False
    else:
        if_tigtog = True
    ...
    if indir and existence:
        if not check_directory_permissions(input):
        ...
        try:
            file_list = [
                ...
                if i.name.endswith((".fna", ".fasta", ".fa"))
            ]
        except FileNotFoundError:
            raise FileNotFoundError(
                f"Input Directory : {input}, does not contain fasta files. Quitting"
            )

        cpus: int = mp_cpu(cpus)
        print(
            f"Running ViralRecall in batch mode on {len(file_list)} files found in {input} and parallelizing across {cpus} cores"
        )
        arg_list: list[tuple] = []
        for i in file_list:
        ...
        with mp.Pool(cpus, maxtasksperchild=1) as pool:
            ...
            print(
                f"Finished running ViralRecall on all files in {input} and the output has been deposited in {project}"
            )

    elif existence and not indir:
        out_base = project / input.stem
        run_program(
            input, out_base, database, window, phagesize, minscore, evalue, minhit
        )
