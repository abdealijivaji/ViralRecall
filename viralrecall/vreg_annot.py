...
def sliding_window_mean(df: pd.DataFrame, window_size) -> pd.Series:
    ...
    for name, grp in df.groupby("contig"):
        df = grp.loc[
            :, ["pstart", "bitscore"]
        ]  # .sort_values(col_B).reset_index(drop=True)
        df["pstart"] = pd.to_datetime(
            grp["pstart"], unit="s", origin="unix", utc=True
        )  # convert starts to datetime index
        rollscr = df.rolling(
            window=window_size, min_periods=3, center=True, on="pstart"
        ).mean()
...
def extract_reg(
    ...
    viral_indices = {
        i: [] for i in filt_contig_list
    }  # dictionary to hold indices of viral regions for each contig
    for name, grp in df.groupby("contig"):
        name = str(name)
        ...
        if len(viral_ranges) > 0:
            ...
            vstart = grp["pstart"].astype("int64")[
                strt_idx
            ]  # start position of the first group

            if len(viral_ranges) == 1:
                end_idx = viral_ranges[0][-1]
                vend = grp["pend"].astype("int64")[
                    end_idx
                ]  # end position of the first group
