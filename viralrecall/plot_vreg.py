...
def plot_vreg(
    annot_tbl: pd.DataFrame, minscore: int, vreg_coords: dict, out_base: Path
) -> None:
    ...
    with PdfPages(out_file) as of:
        for key, coords in vreg_coords.items():
            ...
            plt.axhline(
                minscore, color="red", linestyle="--", label="Min Score Threshold"
            )
            ...
            for i, _ in enumerate(starts):
                ...
                plt.fill_betweenx(
                    y=[0, max(y)], x1=starts[i], x2=ends[i], color="green", alpha=0.1
                )
            of.savefig()
            plt.close()
